...
| `UBX-NAV-PVT` | **Recomendable, no estrictamente obligatorio para el cálculo PPK en sí.** Aporta una solución de posición en tiempo real de menor precisión, útil como verificación en campo, y contiene el campo `iTOW` que se emplea como ancla de sincronización entre el reloj de la Teensy 4.1 y el tiempo GPS absoluto. | [1], apartado 3.15.13 "UBX-NAV-PVT (0x01 0x07)", p. 151 |
| `UBX-RXM-SFRBX` | **Necesario para un post-proceso PPK autocontenido.** Contiene las efemérides transmitidas (posición orbital) que RTKLIB necesita para calcular la posición de cada satélite en el instante de cada medida. Sin este mensaje, el post-proceso dependería por completo de que la fuente de datos de la estación de referencia incluya un fichero de navegación válido para el mismo periodo. | [1], apartado 3.17.9 "UBX-RXM-SFRBX (0x02 0x13)", p. 201 |
...
|---|---|---|---|
| Activar `NAV-PVT` en UART1 (cada época) | `0x20910007` | `0x01` | [1] 6.9.11 (pag. 270) |
| Activar `RXM-RAWX` en UART1 (cada época) | `0x209102A5` | `0x01` | [1] 6.9.11 (pag. 274) |
| Baud rate de UART1 | `0x40520001` | `230400` | [1] 6.9.31 |
...
1. **Verificación empírica**: sobre un fichero de datos crudos multi-constelación
   de referencia [9] (4521 tramas `RXM-RAWX` analizadas), el máximo observado
...
2. **Derivación analítica**: la literatura sobre disponibilidad GNSS reporta
   ...
   constelaciones principales [10], y fuentes de referencia de la industria
   sitúan el máximo práctico en hasta 30-40 satélites simultáneos bajo cielo
   despejado [11]. Distribuyendo esa cifra proporcionalmente entre GPS,
   GLONASS, Galileo y BeiDou, y aplicando el número de señales por satélite
   que soporta el ZED-F9P-15B en cada banda (Tabla 6, [8]), se obtiene un
...
receptor y esta configuración de constelaciones.
...
incluyen flash integrada. El diagrama de bloques específico del ZED-F9P-15B [8]
...
La actualización de firmware del ZED-F9P se realiza mediante u-center (`Tools -> Firmware Update`), conectando el módulo por USB o, en variantes solo-UART como la Ultralight, mediante un adaptador USB-UART, cargando una imagen `.bin` descargada del sitio de u-blox [5].
...
Antes de comprometerse a la secuencia completa de negociación (que implica varias tramas `CFG-VALSET` y sus correspondientes `ACK`), el firmware realiza una **comprobación de bajo coste** del estado actual del receptor mediante una única petición `UBX-CFG-VALGET` (Class `0x06`, ID `0x8B`), consultando en un solo mensaje las tres claves relevantes (`CFG-MSGOUT-UBX_NAV_PVT_UART1`, `CFG-MSGOUT-UBX_RXM_RAWX_UART1`, `CFG-UART1-BAUDRATE`) sobre la capa **RAM** (`layer = 0`), que refleja el valor efectivamente activo en el receptor en ese instante, con independencia de si procede de Flash, BBR o de una configuración de fábrica.

- **Si los tres valores devueltos coinciden** con los deseados (`NAV-PVT` activo, `RAWX` activo, baud rate `230400`), el firmware **omite por completo** la sub-máquina `CONFIGURANDO_GPS` y pasa directamente a `VERIFICANDO_FLUJO`. Esto es coherente con la confirmación de que la Flash del ZED-F9P es interna y persistente (apartado 1.4): si una sesión anterior ya dejó la configuración guardada en Flash, el receptor arrancará con ella ya activa en RAM, y renegociarla en cada vuelo sería un coste innecesario.
...
| 1 | `layer` | `0x00` (RAM) |
| 2-3 | `position` | `0x0000` |
| 4+ | `keys[]` | Las tres keyID de interés, 4 bytes cada una |
...
[1] u-blox, *u-blox F9 HPG 1.51 Interface Description* (UBXDOC-963802114-13124, revisión R01), revisión R01, 8 de noviembre de 2024. https://content.u-blox.com/sites/default/files/documents/u-blox-F9-HPG-1.51_InterfaceDescription_UBXDOC-963802114-13124.pdf
...
[8] u-blox, *ZED-F9P-15B Data Sheet* (UBX-23009090, revisión R04, 10 de enero de 2025) — fuente del diagrama de bloques (Figura 1) que confirma la presencia de memoria flash interna.

[9] rtklibexplorer, "Exploring kinematic single-receiver solutions with RTKLIB and the u-blox F9P" (fichero de datos crudos `rover.ubx` de ejemplo). https://rtklibexplorer.wordpress.com/2021/01/08/exploring-kinematic-single-receiver-solutions-with-rtklib-and-the-u-blox-f9p/

[10] Pan, L., Zhang, X., Li, X., Li, X., Lu, C., Liu, J., Wang, Q. (2019). "Satellite availability and point positioning accuracy evaluation on a global scale for integration of GPS, GLONASS, BeiDou and Galileo." *Advances in Space Research*, 63(9), 2696–2710. DOI: 10.1016/j.asr.2017.07.029.

[11] Mattos, P. G., Pisoni, F. (2018). "Quad-Constellation Receiver: GPS, GLONASS, Galileo, BeiDou." *GPS World*. https://www.gpsworld.com/quad-constellation-receiver-gps-glonass-galileo-beidou/
